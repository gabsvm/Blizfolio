  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 bg-gray-50 dark:bg-[#0f172a] overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0">
        <div className="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-brand-500 rounded-full mix-blend-multiply filter blur-[100px] opacity-50 animate-blob"></div>
      ...
      <div className="glass-card bg-white/70 dark:bg-gray-900/60 p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md border border-white/40 dark:border-white/10 relative z-10 backdrop-blur-xl">
        <div className="text-center mb-8">
          ...
          <p className="text-lg font-bold mt-2 text-gray-800 dark:text-gray-200">{t('register.title')}</p>
        ...
        <div className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400">
          {t('register.footer')} <Link to="/login" className="text-brand-600 dark:text-brand-400 font-bold hover:underline">{t('register.login')}</Link>
        </div>
      </div>
    </div>
  );
